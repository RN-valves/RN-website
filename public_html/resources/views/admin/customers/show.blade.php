...
<div class="card">
   <div class="card-body">
      <div class="card-header">
         Customer Details - {{ $user->user_code }}
         <div class="float-end">
            @can('customer-edit')
            <a class="btn-sm btn btn-primary" href="{{ route('customers.edit',['customer'=>$user]) }}"><i class="bx bx-edit-alt"></i> Edit</a>
            @endcan
            @can('customer-index')
            <a class="btn-sm btn btn-warning" href="{{ route('users.customer_network') }}"> <i class="bx bx-arrow-to-left"></i> Back</a>
            @endcan
         </div>
      </div>
      <div class="row">
         ...
         <div class="col-md-9">
            <table class="table table-hover table-bordered">
               <tr>
                  <th>Name</th>
                  <td>{{ $user->name }} (UType: {{ $user->user_type??'' }})</td>
               </tr>
               <tr>
                  <th>Email</th>
                  <td>{{ $user->email }}</td>
               ...
               @if(!empty($user->sales_user))
               <tr>
                  <th>Sales User</th>
                  <td>{{ $user->sales_user->name??'' }}</td>
               </tr>
               @endif
               ...
               <tr>
                  <th>Login</th>
                  <td>  
                     <a href="{{ route('login.as.customer',$user) }}" target="_blank" class="btn btn-sm btn-warning">Login As User</a>
                  </td>
               </tr>
            </table>
            <div class="row">
            ...
            <div class="table-responsive pt-2" style="max-height: 400px; overflow: scroll;">
               <table class="table table-bordered">
                  <thead>
                     ...
                     <tr>
                        <td class="log_remark">
                           <strong>{{ $log->user->name }}</strong> : <small>{{ $log->created_at->format('d M Y, H:i:s') }}</small> <br>
                           <small class="text-muted">{{ $log->remark??'' }}</small> <br>
