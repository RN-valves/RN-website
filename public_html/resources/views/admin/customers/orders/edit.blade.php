         ...
         <ul class="navbar-nav">
            ...
            <li class="nav-item dropdown">
               ...
               <ul class="dropdown-menu">
                  ...
                  <li><a class="dropdown-item" href="{{ route('orders.edit', ['order'=>$order, 'url_key'=>$subcategory]) }}">{{ $subcategory->name??'' }}</a></li>
                  @endforeach
               </ul>
            </li>
            @endforeach
...
<div class="card mt-2">
   <div class="card-body bg-light py-3">
      <form method="GET" class="row">
         ...
         <div class="col-md-3">
            <button class="btn btn-dark btn-sm w-100">Search / Filter Product</button>
         </div>
      </form>
      <div class="row mt-2">
         <div class="col-md-7">
                  ...
                  @foreach($productsList as $key => $product)
                    ...
                    <input type="hidden" name="product_id" value="{{ $product->id }}">
                      <tr>
                         ...
                         <td class="text-center"><input type="number" step="any" name="price" id="price" value="{{ $product->in_mrp??'' }}" class="text-center"></td>
                         @else
                         <td class="text-center"><input type="number" step="any" name="price" id="price" value="{{ $product->oth_mrp??'' }}" class="text-center"></td>
                         @endif
                         <td class="text-center">
         ...
         <div class="col-md-5" id="">
            <form action="{{ route('orders.AdminorderUpdate', $order) }}" id="saveOrderForm" method="POST" accept-charset="utf-8">
                @csrf
                ...
                <table class="table table-bordered">
                  <tr>
                     ...
                     <td colspan="2" style="text-align: right;">
                        <input type="number" class="form-control" name="shipping_amount" id="shipping_amount" min="0" value="{{ old('shipping_amount', @$order->shipping_amount) }}">
                  ...
                  <tr>
                     ...
                     <td colspan="2" style="text-align: right;">
                        <input type="number" class="form-control" name="discount_amount" step="any" id="discount_amount" value="{{ old('discount_amount', @$order->discount_amount) }}">
                  ...
                  <tr>
                     ...
                     <td colspan="2" style="text-align: right;">
                        <select name="shipping_charge_id" class="form-control">
                           ...
                           <option value="{{ $address->id }}" @selected(old('shipping_charge_id', @$order->shipping_charge_id)==$address->id)>{{ $address->mobile??'' }}-{{ $address->zipcode??'' }}</option>
                           @endforeach
                        </select>
                  ...
                  <tr>
                     ...
                     <td colspan="2" style="text-align: right;">
                        <select name="payment_term" class="form-control">
                           <option value="100% Advanced" @selected(old('payment_term', @$order->payment_term)=='100% Advanced')>100% Advanced</option>
                           <option value="Credit" @selected(old('payment_term', @$order->payment_term)=='Credit')>Credit</option>
                        </select>
                  ...
                  <tr>
                     ...
                     <td colspan="2" style="text-align: right;">
                        <textarea class="form-control" name="note" rows="3" placeholder="Query">{{ old('note', @$order->note) }}</textarea>
