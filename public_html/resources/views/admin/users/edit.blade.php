      ...
      <form action="{{ route('users.update', @$user) }}" method="POST" class="row g-3">
         ...
         <div class="col-lg-12 mb-3">
            <label for="roles_select" style="width:100%;">
              ...
              <select class="js-example-basic-multiple js-states form-control" id="roles_select" name="roles">
                  ...
                  <option value="{{ $role }}" {{ (in_array($role, $user->getRoleNames()->all())) ? 'selected' : '' }}>{{ $role }}</option>
                  @endforeach
              </select>
            </label>
         </div>
         {{-- <div class="col-lg-6 mb-3">
            <label for="select_user" style="width:100%;">
              Select AssignTo/Reporting User 
              <select class="js-example-basic-multiple js-states form-control" id="select_user" name="reporting_ids[]" multiple="">
                  @foreach ($employees as $employees)
                  <option value="{{ $employees->id }}" @foreach($user->reporting_users as $reporting){{$reporting->reporting_id == $employees->id ? 'selected': ''}}   @endforeach> {{ $employees->name }}</option>
                  @endforeach
              </select>
         ...
         <div class="col-md-4">
            <div class="form-group">
               <label for="user_code">User Code (Opt)</label>
