            ...
            <div class="col-lg-12">
               <div class="">
                  <table class="table table-bordered">
                     <tr>
                        <th class="text-center" style="background: #cacdcf;">Add New Color</th>
                     </tr>
                  </table>
                  <form class="row" method="POST" action="{{ route('products.addNewColor', $product) }}" enctype="multipart/form-data">
                     @csrf
                     <div class="col-lg-12">
                        @if (count($errors) > 0)
                     ...
                     <div class="col-lg-4 form-group mb-1">
                        <label class="mb-1">Select Color</label>
                        <select class="js-states form-control" id="color_name" name="color_name">
                        @foreach ($colors??'' as $color)
                        <option value="{{ $color->name??'' }}" @selected(old('color_name')==$color->name??'')>{{ $color->name??'' }}</option>
                        @endforeach
                        </select>
                     ...
                     <div class="col-lg-4 form-group mb-1">
                        ...
                        <input type="text" name="article" value="{{ old('article') }}" class="form-control shadow-none" placeholder="Enter Value" autocomplete="off">
                     ...
                     <div class="col-lg-4 form-group pt-2">
                        <label class="mb-1">Stock (Pcs)</label>
                     ...
                     <div class="col-md-3 form-group pt-2">
                        <button type="submit" class="btn btn-success btn-sm w-100"> <i class="bx bx-folder-plus"></i> Add New Color</button>
                     </div>
                  </form>
               </div>
               <div class="table-responsive pt-3">
                  <table class="table table-bordered table-hover table-striped">
                     ...
                     <tr>
                        ...
                        <td>{{ $colorPro->in_v1_mrp??'' }}</td>
                        <td>{{ $colorPro->oth_mrp??'' }}</td>
                        <td>{{ $colorPro->oth_selling??'' }}</td>
                        <td>{{ $colorPro->oth_v1_mrp??'' }}</td>
                        <td>
